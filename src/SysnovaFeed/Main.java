...
public class Main extends javax.swing.JFrame {
    ...
    public Main() {
        ...
        item.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
             ...
             //   JOptionPane.showMessageDialog(menu, "Copy Button Clicked : "+compoundJtable_tableLine.get(0));
                CompoundCopy _compoundCopy=new CompoundCopy((String) compoundJtable_tableLine.get(0),compound_tableListData);
                _compoundCopy.setVisible(true);
                _compoundCopy.setLocation(200, 200);
            }
        });
             menu.add(item);
        compound_jTable.setComponentPopupMenu(menu);
    ...
    public void findVersion(){
          try{
              boolean findVersion = sqlManager.checkDatabase();
             
              if(findVersion == false){
                  try{
                
                        boolean isUpdated = sqlManager.updateDatabase();
                            if(isUpdated){
                                JOptionPane.showMessageDialog(null, "Version Updated Successfull", "Update Successfully", JOptionPane.INFORMATION_MESSAGE);
                            }else{
                                JOptionPane.showMessageDialog(null, "Version Update Failed", "Failed", JOptionPane.INFORMATION_MESSAGE);
                            }
   ...
   private CategoryDataset createDataset( )
   {
      ...
      try {
            ...
            if(CFList.size()>0){
                Vector line=(Vector)CFList.get(0);
...
//            userNameLabel3.setText("Hello, "+sqlManager.getUserName());
    ...
    int compountJtable_selectedRow=-1;
    Vector compoundJtable_tableLine;
    private void compound_jTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_compound_jTableMouseClicked
        ...
        if (evt.getClickCount() == 1) {
            compountJtable_selectedRow = ((JTable) evt.getSource()).rowAtPoint(evt.getPoint());
            compoundJtable_tableLine =(Vector) compound_tableListData.get(compountJtable_selectedRow);
        }
